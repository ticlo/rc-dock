...
export function saveLayoutData(layout, saveTab, afterPanelSaved) {
    ...
    function savePanelData(panelData) {
        ...
        let { id, size, activeId, collapsed, headerSize, preferredWidth, preferredHeight, ignorePreferredSize, group, localGroup, x, y, z, w, h, dockParent, panelIndex, tabIndex, panelLock, toggleFloatingDisabled, tabPosition = "top" } = panelData;
        let savedPanel;
        if (panelData.parent.mode === 'float' || panelData.parent.mode === 'window') {
            savedPanel = { id, size, tabs, collapsed, headerSize, group, preferredWidth, preferredHeight, localGroup, ignorePreferredSize, tabPosition, activeId, x, y, z, w, h, dockParent, panelIndex, tabIndex, panelLock, toggleFloatingDisabled };
        }
        else {
            let { needSetSize } = panelData;
            savedPanel = { id, size, tabs, collapsed, headerSize, group, preferredWidth, preferredHeight, localGroup, ignorePreferredSize, tabPosition, activeId, x, y, z, w, h, needSetSize, dockParent, panelIndex, tabIndex, panelLock, toggleFloatingDisabled };
        }
        if (afterPanelSaved) {
...
export function loadLayoutData(savedLayout, defaultLayout, loadTab, afterPanelLoaded) {
    ...
    function loadPanelData(savedPanel) {
        let { id, size, activeId, x, y, z, w, h, collapsed, headerSize, preferredWidth, preferredHeight, ignorePreferredSize, group, localGroup, dockParent, panelIndex, tabIndex, needSetSize, panelLock, toggleFloatingDisabled, tabPosition = "top" } = savedPanel;
        let tabs = [];
        ...
        let panelData;
        if (w || h || x || y || z) {
            panelData = { id, size, activeId, group, localGroup, tabPosition, collapsed, headerSize, preferredWidth, preferredHeight, ignorePreferredSize, x, y, z, w, h, tabs, dockParent, panelIndex, tabIndex, panelLock, toggleFloatingDisabled };
        }
        else {
            panelData = { id, size, activeId, group, localGroup, tabPosition, collapsed, headerSize, preferredWidth, preferredHeight, ignorePreferredSize, tabs, needSetSize, dockParent, panelIndex, tabIndex, panelLock, toggleFloatingDisabled };
        }
